(function () {
  ...
  function renderUsers() {
    ...
    Wc.$("userRows").innerHTML = users.length ? users.map(function (u) { return '<tr><td><b>' + Wc.esc(u.email) + '</b><small>' + Wc.esc(u.accountType) + '</small></td><td>' + Wc.esc(fmt(u.joinedAt)) + '</td><td>' + Wc.esc(fmt(u.lastActiveAt)) + '</td><td>' + u.jobs + '</td><td>' + u.inspections + '</td><td><span class="wc-admin-stage">' + Wc.esc(u.workflowStage) + '</span></td></tr>'; }).join("") : '<tr><td colspan="6" class="sel-empty">No accounts match this search.</td></tr>';
  }
  function render(data) {
    overview = data; var t = data.totals;
    Wc.$("aUsers").textContent = t.users; Wc.$("aGoogle").textContent = t.googleUsers; Wc.$("aGuests").textContent = t.guestUsers; Wc.$("aJobs").textContent = t.jobs; Wc.$("aInspections").textContent = t.inspections; Wc.$("aActive").textContent = t.activeSevenDays;
    renderUsers();
    Wc.$("recentJobs").innerHTML = data.recentJobs.length ? data.recentJobs.map(function (j) { return '<div class="sel-row"><div class="sel-row__body"><div class="sel-row__title">Sample ' + Wc.esc(j.sampleNo) + (j.jobName ? ' · ' + Wc.esc(j.jobName) : '') + '</div><div class="sel-row__sub">' + Wc.esc(fmt(j.createdAt)) + ' · Checklist ' + j.checklistDone + '/9 · ' + Wc.esc(j.status) + '</div></div></div>'; }).join("") : '<p class="sel-empty">No jobs yet.</p>';
